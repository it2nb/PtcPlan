    ...
    <div class="printPage3x2">
      <v-row no-gutters class="mb-2">
        ...
        <v-col
          cols="8"
          align-self="end"
          class="text-center font25 font-weight-bold"
        >
          บันทึกข้อความ
        </v-col>
        <v-col
          ...
          <table>
            <tr>
              ...
              <td class="pl-3 font17">
                ...
                2. แบบตรวจสอบรายการคำขอจัดซื้อจัดจ้าง รหัส DB-{{
                  parseInt(disburseID)
                }}
              </td>
            </tr>
        ...
        <v-col cols="12" class="pt-1 font17">
          ...
          }}<span class="font17" v-if="disburse.disburseSubtype == 'ซื้อ'">{{
          ...
          จำนวน {{ disburselists.length }} รายการ โดยมีค่าใช้จ่ายเป็นเงินจำนวน
        ...
        <v-col cols="12" class="pt-1 font17">
          ...
          จึงขออนุญาตดำเนินการและขออนุมัติงบประมาณเป็นเงินจำนวน
          {{ moneyFormat(disburse.disburseMoney) }} บาท ({{
            thaiBaht(disburse.disburseMoney)
        ...
        <v-col cols="12" class="mt-10">
          <v-row no-gutters v-if="disburse.disburseType == 'ค่าใช้จ่าย'">
            <v-col
              align-self="start"
              class="text-center font17"
              v-if="disburse.userID != disburse.departmentHeadUserID"
            >
          ...
          <v-row no-gutters v-if="disburse.disburseType == 'โครงการ'">
            ...
            >
              <img
                :src="pjdepartmentSign + '?t=' + new Date()"
                style="max-width: 100px; max-height: 30px"
                v-if="pjdepartmentSign"
              /><br />
            ...
            <v-col align-self="start" class="text-center font17" v-else>
              <img
                ...
                style="max-width: 100px; max-height: 30px"
                v-if="
                  pjdepartmentSign && disburse.departmentSign == disburse.userID
                "
              /><img
                :src="disburseSign + '?t=' + new Date()"
                style="max-width: 100px; max-height: 30px"
    ...
    <div class="printPage3x2">
      <v-row no-gutters class="mb-2">
        <v-col
          cols="12"
          align-self="end"
          class="text-center font19 font-weight-bold"
        >
          แบบตรวจสอบรายการคำขอจัดซื้อจัดจ้าง <br />รหัส DB-{{
            parseInt(disburseID)
          }}
        </v-col>
        <v-col
          cols="12"
          class="font17 text-center"
          v-if="disburse.disburseType == 'ค่าใช้จ่าย'"
        >
        ...
        <v-col cols="12" class="mt-3 font17">
          ...
          <span class="font17" v-if="disburse.disburseParcCheck == 'ถูกต้อง'"
    ...
    <div class="printPage3x2">
      <v-row no-gutters class="mb-2">
        ...
        <v-col cols="5" class="font17">
          ...
          <v-row no-gutters>
            <v-col class="font17"> - {{ disburse.expenseName }} </v-col>
            <v-col class="font17 text-right pr-3">
              {{ moneyFormat(disburse.disburseMoney) }} บาท
            </v-col>
          </v-row>
          <v-row no-gutters>
            <v-col class="font17"> &emsp;&emsp;รวม </v-col>
            <v-col class="font17 text-right pr-3">
              {{ moneyFormat(disburse.disburseMoney) }} บาท
            </v-col>
          </v-row>
        ...
        <v-col cols="7" class="font17">
          <table width="100%" class="tableNormal">
            <tr>
              <td class="font17 text-center">ผลิตผลที่คาดว่าจะได้</td>
            </tr>
            <tr>
              <td class="font17 px-3">
                <pre class="font17">{{ disburse.disburseProduct }}</pre>
              </td>
            </tr>
          </table>
        </v-col>
        ...
        <v-col
          cols="6"
          class="font17 text-center mt-5"
          v-else-if="disburse.disburseType == 'ค่าใช้จ่าย'"
        >
          <img
            :src="disburseSign + '?t=' + new Date()"
            style="max-width: 100px; max-height: 25px"
              ...
              <tr>
                <th colspan="7" class="font17 text-center font-weight-bold">
                  <div class="text-right font16">
                    รหัสคำขอจัดซื้อ DB-{{ parseInt(disburseID) }}
                  </div>
                  ประมาณการค่าใช้จ่าย (ค่าใช้สอย / ค่าตอบแทน / ค่าวัสดุ)
                </th>
              </tr>
              <tr>
                <th class="font17 font-weight-bold">ลำดับ</th>
                <th class="font17 font-weight-bold">รายการ</th>
                <th colspan="2" class="font17 font-weight-bold">
                  จำนวน<br />หน่วย
                </th>
                <th class="font17 font-weight-bold">ราคาต่อหน่วย</th>
                <th class="font17 font-weight-bold">รวมเงิน</th>
                <th class="font17 font-weight-bold">หมายเหตุ</th>
              </tr>
            </thead>
            <tbody>
              <tr>
              ...
              <tr v-if="disburse.disburseExcludeVat == 1">
                <td colspan="5" class="font17 text-right">
                  ภาษีมูลค่าเพิ่ม 7%
                </td>
                <td class="font17 text-right">
                  {{
                    moneyFormat(
              ...
              <tr>
                <td colspan="5" class="font17 text-right font-weight-bold">
                  รวมเงิน
                </td>
                <td class="font17 text-right font-weight-bold">
      ...
      if (disburseQuery.message == "Success") {
        ...
        if (this.disburse.departmentHeadUserID) {
          this.disburseDepartmentSign = await this.getDepartmentSignature(
            this.disburse.departmentHeadUserID
          );
        }
        if (this.disburse.departmentSign) {
          this.pjdepartmentSign = await this.getDepartmentSignature(
            this.disburse.departmentSign
          );
        }
