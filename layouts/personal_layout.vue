          ...
          <v-list-item-content>
            <v-row class="pa-0 col-12 align-center">
              <v-col cols="9" class="text-right">
                <div class="fontBold">
                  {{ personal.personalName+" "+personal.personalSer }}
                </div>
                <div class="mt-1">
                  <v-btn
                    x-small
...
export default {
  ...
  mounted() {
    let loginpersonal = JSON.parse(sessionStorage.getItem("loginuser")) || {type: ''}
    if(loginpersonal.type == 'Personal') {
      this.personal = loginpersonal.personal
    } else {
      this.$router.replace('/')
    }
  },
