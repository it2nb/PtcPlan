      ...
      <v-col cols="12">
        <v-card elevation="1">
          <v-card-title class="ptcBg white--text">
            <b>ค่าใช้จ่าย ประจำปีงบประมาณ พ.ศ.{{ parseInt(expenseplanYear)+543 }}</b>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            ...
            >
              ...
              <template v-slot:item.expenseplanID="{ item }">
                <div  class="text-no-wrap">{{ expenseplans.indexOf(item)+1 }}</div>
              </template>
              ...
              <template v-slot:item.disburseMoney="{ item }">
                ...
                <div class="py-2 text-no-wrap">
                  ...
                  <div>
                    <v-progress-linear
                      :value="parseFloat(item.disburseMoney)/parseFloat(budgetSum.disburseMoney)*100"
                      background-color="white"
                      color="red lighten-3"
                      height="10"
                      reverse
                      v-if="budgetSum.disburseMoney>0"
                    >
                      <!-- <template v-slot:default="{}">
                        <div class="pa-0 col-12 text-right text-no-wrap">
                          <small>{{ moneyFormat(parseFloat(item.disburseMoney)/parseFloat(budgetSum.disburseMoney)*100) }}%</small>
                        </div>
                      </template> -->
                    </v-progress-linear>
                  </div>
  ...
  methods: {
    async getExpenseplans() {
      ...
      let params = {
        ...
        disburseYear: this.expenseplanYear,
        fn: 'getSummaryByExpenseYear'
      }
      let result = await this.$axios.$get('disburse.php', {params})
      if(result.message === 'Success') {
        let expenseplans = JSON.parse(JSON.stringify(result.disburse))
